        ...
        if ( event.getEntity() instanceof Player) {
            ...
            if ( !player.level().isClientSide ) {
                if ( event.getItem().getItem().equals(BigFishItems.COOKED_SLIMY_EEL.get()) ) {
                    int addedFood = (int)(BigFishCommonConfig.EEL_HUNGER_MIN.get() + Math.random() * (BigFishCommonConfig.EEL_HUNGER_MAX.get() - BigFishCommonConfig.EEL_HUNGER_MIN.get()));
                    foodData.setFoodLevel(foodData.getFoodLevel() + addedFood);
                    float addedSaturation = (float)(BigFishCommonConfig.EEL_SATURARION_MIN.get() + Math.random() * (BigFishCommonConfig.EEL_SATURARION_MAX.get() - BigFishCommonConfig.EEL_SATURARION_MIN.get()));
                    foodData.setSaturation(foodData.getSaturationLevel() + addedSaturation);
                }
                if ( event.getItem().getItem().equals(BigFishItems.COOKED_ANGLERFISH.get()) ) {
                    player.addEffect(new MobEffectInstance(MobEffects.HEALTH_BOOST, BigFishCommonConfig.ANGLERFISH_BOOST_TIME.get(), BigFishCommonConfig.ANGLERFISH_BOOST_POWER.get()));
                }
            }
        }
    }
